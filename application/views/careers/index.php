...
<?= form_open('', array('class'=>'form-search')); ?>	
	<?= anchor('careers/create','Agregar',array('class'=>'btn btn-primary'))?>
<?= form_close();?>
	...
	<tbody>
		...
		<tr class="success">
			<td><?= anchor('careers/edit/'.$register->id,$register->id,array('title'=>'Click para modificar o eliminar'));  		?></td>
			<td> <?= $register->code  ?>    </td>
			<td> <?= $register->name  ?>    </td>
